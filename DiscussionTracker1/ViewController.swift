...
class Discussion{
    ...
    convenience init(){//populate with specific classroom for testing and debugging
...
//would-be model for connections and later decided to keep that logic only in the view (this is still implicit in the contribution list)
/*
...
struct Contributor:Hashable{
    ...
    var hashValue: Int{ // currently, only unique iff first & last are same. Could include color, but relying on random # for hashing is probably not good idea. Todo: Unique ID?
...
// don't mind this - this is necessary for the struct "Contributor" to be hashable.
func ==(lhs:Contributor, rhs:Contributor)-> Bool{
    return (lhs.firstName == rhs.firstName && lhs.lastName == rhs.lastName)
}

//  We never use a title in the current version of the app - this is a bit of an aspirational model - for when distinct classrooms will have distinct names.
struct Classroom{
    var title:String;
...
class ViewController: UIViewController {
    ...
    //Initialize a button for a specific contributor.
    func button(fromContributor cont: Contributor)-> UIButton{
        ...
        myButton.frame = CGRectMake(0.0, 0.0, 150, 150) //!Magic number -  button size const
    ...
    //tell view to connect buttons (however we're doing that now)
    func onlyConnect(fromButton fromBut : UIButton, toButton toBut:UIButton){
        //TODO - move lines to outside of button, rather than drawing from centers.
    ...
    //TODO: Bad things will happen here - can we couple model objects to buttons more elegantly?
